...
class OpNoiseEstim():
    ...
    def __init__(
            self, signal=None, flags=None, detmask=1, commonmask=1, out=None,
            maskfile=None, mapfile=None, pol=True, nbin_psd=1000,
            lagmax=10000, stationary_period=86400., nosingle=False, nocross=True,
            calibrate_signal_estimate=False, nsum=10, naverage=100,
            apply_intervals=False, pairs=None, save_cov=False):
        """
        Args:
    ...
    def exec(self, data):
        ...
        for obs in data.obs:
            ...
            if len(tod.local_dets) < len(tod.detectors):
                raise RuntimeError('Noise estimation does not work on '
                                   'detector-split data')
            dets = {}
            ...
            if self._apply_intervals:
                ...
                intervals = [Interval(
                    start=timestamps[0], stop=timestamps[-1],
                    first=0, last=timestamps.size - 1)]

            self.subtract_signal(
                tod, comm, masksampler, mapsampler, intervals)
            ...
            for det1, det2 in pairs:
                ...
                self.process_noise_estimate(
                    ...
                    timestamps, fsample, comm, fileroot, det1, det2,
                    intervals)
    ...
    def highpass_signal(self, tod, comm, intervals):
        ...
        if comm.rank == 0:
            print('TOD high-passed in {:.2f} s'.format(
                stop_signal_subtract - start_signal_subtract),
                flush=True)
        return

    def subtract_signal(self, tod, comm, masksampler, mapsampler,
                        intervals):
    ...
    def save_psds(self, binfreq, all_psds, all_times, det1, det2, fsample,
        ...
        if self._save_cov:
            ...
            cols.append(pf.Column(
                name='HITS', format='{}J'.format(nsamp),
                array=np.ascontiguousarray(all_cov[:, 0, :])))
            cols.append(
                pf.Column(name='COV', format='{}E'.format(nsamp),
    ...
    def process_downsampled_noise_estimate(
            self, timestamps, fsample, signal1, signal2, flags, gapflags_nsum,
            local_intervals, my_psds1, my_cov1, comm):
        ...
        if signal2 is None:
            result = autocov_psd(
                ...
                self._stationary_period, fsample / self._nsum, comm=comm,
    ...
    def process_noise_estimate(
        ...
        if signal2 is None:
            result = autocov_psd(
                ...
                self._stationary_period, fsample, comm=comm,
                return_cov=self._save_cov)
        else:
            result = crosscov_psd(
        ...
        if self._nsum > 1:
            my_psds1, my_cov1, my_psds2, my_cov2 = \
                self.process_downsampled_noise_estimate(
                    timestamps, fsample, signal1, signal2, flags, gapflags_nsum,
                    local_intervals, my_psds1, my_cov1, comm)
        ...
        if binfreq10 is None:
            ...
            if self._nsum > 1:
                ...
                for psd1, psd2 in zip(my_binned_psds1, my_binned_psds2):
                    my_binned_psds.append(np.hstack([psd2[ind2],
                                                     psd1[ind1]]))
            else:
                binfreq0 = binfreq10
        ...
        if binfreq0 is not None and np.any(binfreq != binfreq0):
            raise Exception(
                ...
                'len(my_psds)={}'.format(
                    comm.rank, binfreq0.size, binfreq.size, binfreq0,
