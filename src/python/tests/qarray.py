...
class QarrayTest(MPITestCase):
    ...
    def setUp(self):
        ...
        self.vec2 = np.array([
            [0.57734543, 8.30271255, 5.75831218],
            [1.57734543, 3.30271255, 0.75831218]])
    ...
    def test_rotate_qarray(self):
        ...
        np.testing.assert_array_almost_equal(
            my_rot_result,
            np.vstack([self.rot_by_q1, self.rot_by_q2]).reshape((2, 3)))
    ...
    def test_pow(self):
        ...
        np.testing.assert_array_almost_equal(
            qarray.pow(self.qeasy, 3.0),
            np.array([[0.672, 0.672, 0.224, 0.216],
                      [0.672, 0.672, 0.224, 0.216]]))
        np.testing.assert_array_almost_equal(
    ...
    def test_depths(self):
        ...
        np.testing.assert_equal(qarray.inv(self.q1).shape, (4,))
        np.testing.assert_equal(
            qarray.inv(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(np.shape(qarray.amplitude(self.vec)), ())
        np.testing.assert_equal(
            np.shape(qarray.amplitude(np.atleast_2d(self.vec))), (1,))

        np.testing.assert_equal(qarray.norm(self.q1).shape, (4,))
        np.testing.assert_equal(
            qarray.norm(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(qarray.rotate(self.q1, self.vec).shape, (3,))
        np.testing.assert_equal(
            qarray.rotate(np.atleast_2d(self.q1), self.vec).shape, (1, 3))
        np.testing.assert_equal(
            qarray.rotate(self.q1, np.atleast_2d(self.vec)).shape, (1, 3))

        q = qarray.norm(np.array([[2., 3, 4, 5], [6., 7, 8, 9]]))
        time = np.array([0., 9])
        np.testing.assert_equal(np.shape(qarray.slerp(0, time, q)), (4,))
        np.testing.assert_equal(np.shape(qarray.slerp([0], time, q)), (1, 4))

        np.testing.assert_equal(qarray.exp(self.q1).shape, (4,))
        np.testing.assert_equal(
            qarray.exp(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(qarray.ln(self.q1).shape, (4,))
        np.testing.assert_equal(
            qarray.ln(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(qarray.pow(self.q1, 2).shape, (4,))
        np.testing.assert_equal(
            qarray.pow(np.atleast_2d(self.q1), 2).shape, (1, 4))
        np.testing.assert_equal(
            qarray.pow(self.q1, [2]).shape, (1, 4))
        np.testing.assert_equal(
            qarray.pow(np.atleast_2d(self.q1), [2]).shape, (1, 4))

        np.testing.assert_equal(qarray.rotation([0, 0, 1], np.pi).shape, (4,))
        np.testing.assert_equal(
            qarray.rotation([[0, 0, 1]], np.pi).shape, (1, 4))
        np.testing.assert_equal(
            qarray.rotation([0, 0, 1], [np.pi]).shape, (1, 4))
        np.testing.assert_equal(
            qarray.rotation([[0, 0, 1]], [np.pi]).shape, (1, 4))

        ret1 = qarray.to_axisangle(self.q1)
        ret2 = qarray.to_axisangle(np.atleast_2d(self.q1))
        ...
        np.testing.assert_equal(np.shape(qarray.from_angles(0, 0, 0)), (4,))
        np.testing.assert_equal(np.shape(qarray.from_angles([0], 0, 0)), (1, 4))
        np.testing.assert_equal(
            np.shape(qarray.from_angles([0], [0], [0])), (1, 4))

        ret1 = qarray.to_angles(self.q1)
        ret2 = qarray.to_angles(np.atleast_2d(self.q1))
