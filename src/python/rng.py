...
def random(samples, counter=(0,0), sampler="gaussian", key=(0,0)):
    ...
    if sampler == "gaussian":
        ...
        ret = rng_dist_uniform_01(samples, key[0], key[1], counter[0],
            counter[1])
    elif sampler == "uniform_m11":
        ret = rng_dist_uniform_11(samples, key[0], key[1], counter[0],
            counter[1])
    elif sampler == "uniform_uint64":
        ret = rng_dist_uint64(samples, key[0], key[1], counter[0], counter[1])
    else:
        raise ValueError("Undefined sampler. Choose among: gaussian, "
            "uniform_01, uniform_m11, uniform_uint64")
    return ret


@timing.auto_timer
def random_mt(blocks, samples, counter=[(0,0)], sampler="gaussian",
    key=[(0,0)]):
    """
    High level interface to internal Random123 library.
