...
const size_t MPITOASTShmemTest::n = 10;


TEST_F(MPITOASTShmemTest, instantiate) {
    toast::mpi_shmem <double> shmem;
    shmem.allocate(n);

    toast::mpi_shmem <double> shmem2(n);
...
TEST_F(MPITOASTShmemTest, access) {
    MPI_Comm comm = MPI_COMM_WORLD;
    int rank;
    MPI_Comm_rank(comm, &rank);

    toast::mpi_shmem <double> shmem(n);
