    ...
    int rank)
{
    ...
    if (first_kappa > nkappa - 1)
        return;

    int64_t last_kappa = first_kappa + nkappa_task;
    if (last_kappa > nkappa)
        last_kappa = nkappa;
...
#pragma omp parallel for schedule(static, 10)
    for (int64_t ikappa = first_kappa; ikappa < last_kappa; ++ikappa)
    {
        ...
        phi[ikappa - first_kappa] =
            (1. + 1.802 * kkl - 0.254 * pow(kkl, slope1)) * exp(-kkl * kkl) * pow(k * k + kappa0sq, slope2);
    }
    ...
    if (atm_verbose())
    {
        ...
        for (int64_t ikappa = first_kappa; ikappa < last_kappa; ++ikappa)
        {
            f << ikappa << " " << kappa[ikappa - first_kappa] << " " << phi[ikappa - first_kappa] << std::endl;
        }
        f.close();
    }
    ...
    if (first_kappa == 0)
        phi[0] /= 2;
    if (last_kappa == nkappa)
        phi[last_kappa - first_kappa - 1] /= 2;
...
#pragma omp parallel for schedule(static, 10)
    ...
    {
        double r = rmin_kolmo + (exp(ir * nri * tau) - 1) * enorm *
        ...
        else
        {
            for (int64_t ikappa = first_kappa; ikappa < last_kappa - 1; ++ikappa)
            {
                ...
                val += .5 * (phi1 + phi2) * rinv *
                       (k1 * cos(k1 * r) - k2 * cos(k2 * r) - rinv * (sin(k1 * r) - sin(k2 * r)));
            }
            val /= r;
        }
        kolmo_x[ir] = r;
    ...
    double const *kolmo_y)
{
    ...
    {
        ir = low + 0.5 * (high - low);
        if (kolmo_x[ir] <= r and r <= kolmo_x[ir + 1])
            break;
        if (r < kolmo_x[ir])
            high = ir;
        else
...
#pragma omp for schedule(static, 10)
        for (int64_t icol = 0; icol < nelem; ++icol)
        {
            ...
            for (int64_t irow = icol; irow < nelem; ++irow)
            {
                ...
                if (fabs(colcoord[0] - rowcoord[0]) > rcorr)
                    continue;
                if (fabs(colcoord[1] - rowcoord[1]) > rcorr)
                    continue;
                if (fabs(colcoord[2] - rowcoord[2]) > rcorr)
                    continue;
...
#pragma omp critical
    ...
    double tot_mem =
        (nelem * sizeof(int) + nnz * (sizeof(int) + sizeof(double))) / pow(2.0, 20.0);
    ...
    int rank)
{
    ...
    for (int itry = 0; itry < ntry; ++itry)
    {
        ...
        if (chol.chcommon->status != CHOLMOD_OK)
        {
            ...
            if (itry < ntry - 1)
            {
                ...
                throw std::runtime_error("cholmod_l_factorize failed.");
        }
        else
        {
            break;
    ...
    tot_mem = (nelem * sizeof(int) + nnz * (sizeof(int) + sizeof(double))) / pow(2.0, 20.0);
