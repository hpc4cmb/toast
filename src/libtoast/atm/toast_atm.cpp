...
namespace toast { namespace atm {
    ...
    atmsim::atmsim( double azmin, double azmax, double elmin, double elmax, double tmin, double tmax, double lmin_center, double lmin_sigma, double lmax_center, double lmax_sigma, double w_center, double w_sigma, double wdir_center, double wdir_sigma, double z0_center, double z0_sigma, double T0_center, double T0_sigma, double zatm, double zmax, double xstep, double ystep, double zstep, long nelem_sim_max, int verbosity, MPI_Comm comm, int gangsize, double fnear ) : azmin(azmin), azmax(azmax), elmin(elmin), elmax(elmax), tmin(tmin), tmax(tmax), lmin_center(lmin_center), lmin_sigma(lmin_sigma), lmax_center(lmax_center), lmax_sigma(lmax_sigma), w_center(w_center), w_sigma(w_sigma), wdir_center(wdir_center), wdir_sigma(wdir_sigma), z0_center(z0_center), z0_sigma(z0_sigma), T0_center(T0_center), T0_sigma(T0_sigma), zatm(zatm), zmax(zmax), xstep(xstep), ystep(ystep), zstep(zstep), nelem_sim_max(nelem_sim_max), verbosity(verbosity), comm(comm), gangsize(gangsize), fnear(fnear) {
    ...
    void atmsim::simulate( bool save_covmat ) {
    ...
    void atmsim::get_slice( long &ind_start, long &ind_stop ) {
    ...
    void atmsim::observe( double *t, double *az, double *el, double *tod, long nsamp, double fixed_r ) {
...
#pragma omp parallel for schedule(static, 100)
    ...
    void atmsim::initialize_kolmogorov() {
...
#pragma omp parallel for schedule(static, 10)
    ...
    double atmsim::kolmogorov( double r ) {
...
#pragma omp parallel for schedule(static, 10)
    ...
    bool atmsim::in_cone( double x, double y, double z ) {
    ...
    void atmsim::ind2coord( long i, double *coord ) {
    ...
    long atmsim::coord2ind( double x, double y, double z ) {
    ...
    double atmsim::interp( std::vector<double> &realization, double x, double y, double z, std::vector<long> &last_ind, std::vector<double> &last_nodes ) {
    ...
    void atmsim::build_covariance( long ind_start, long ind_stop, bool save_covmat, double scale ) {
    ...
    double atmsim::cov_eval( double *coord1, double *coord2, double scale ) {
    ...
    void atmsim::sqrt_covariance( El::DistMatrix<double> *cov, long ind_start, long ind_stop, bool save_covmat, int near ) {
	...
	if ( rank_gang == 0 && verbosity > 0 ) {
	  std::cerr << std::endl;
	  std::cerr << "Gang # " << gang << " near = " << near << " Cholesky decomposition done in " << t2-t1 << " s. N = " << nelem_slice << " ntask = " << ntask << " nthread = " << nthread << std::endl;
	}
	...
	if ( rank_gang == 0 && verbosity > 0 ) {
	  std::cerr << std::endl;
	  std::cerr << "Gang # " << gang << " near = " << near << " Cholesky decomposition failed, falling back to eigenvalue decomposition." << std::endl;
	}
	...
	if ( rank_gang == 0 && verbosity > 0 ) {
	  std::cerr << std::endl;
	  std::cerr << "Gang # " << gang << " near = " << near << " Eigenvalue decomposition done in " << t2-t1 << " s." << std::endl;
	}
	...
	if ( rank_gang == 0 && verbosity > 0 )
	  std::cerr << "Gang # " << gang << " near = " << near << " Maximum eigenvalue = " << evalmax << ", minimum = " << evalmin << ", rcond = " << evalmin / evalmax << std::endl;
	...
	if ( verbosity > 0 ) {
	  ...
	  if ( rank_gang == 0 ) {
	    std::cerr << "Gang # " << gang << " near = " << near << " Zeroed " << nbad_tot << " / " << nelem_slice << " eigenvalues as negative." << std::endl;
	  }
	}
    ...
    void atmsim::apply_covariance( El::DistMatrix<double> *cov, std::vector<double> &realization, long ind_start, long ind_stop, int near ) {
	...
	p[i] -= mean;
      ...
      if ( rank_gang == 0 && verbosity > 0 ) {
	std::cerr << std::endl;
	std::cerr << "Gang # " << gang << " near = " << near << " Realization slice (" << ind_start <<" -- "<< ind_stop << ") constructed in "
		  << t2-t1 << " s." << std::endl;
      }
