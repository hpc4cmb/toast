...
class QarrayTest(MPITestCase):
    ...
    def test_slerp(self):
        q = qa.norm(np.array([[2., 3, 4, 5], [6., 7, 8, 9]]))
        time = np.array([0., 9])
    ...
    def test_fromvectors(self):
        ...
        q = qa.from_vectors(np.tile(v1, 3).reshape(-1, 3),
    ...
    def test_depths(self):
        ...
        np.testing.assert_equal(
            qa.mult(np.atleast_2d(self.q1), self.q2).shape, (1, 4))

        np.testing.assert_equal(qa.inv(self.q1).shape, (4,))
        np.testing.assert_equal(
            qa.inv(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(np.shape(qa.amplitude(self.q1)), ())
        np.testing.assert_equal(
            np.shape(qa.amplitude(np.atleast_2d(self.q1))), (1,))

        np.testing.assert_equal(qa.norm(self.q1).shape, (4,))
        np.testing.assert_equal(
            qa.norm(np.atleast_2d(self.q1)).shape, (1, 4))
        ...
        np.testing.assert_equal(
            qa.exp(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(qa.ln(self.q1).shape, (4,))
        np.testing.assert_equal(
            qa.ln(np.atleast_2d(self.q1)).shape, (1, 4))

        np.testing.assert_equal(qa.pow(self.q1, 2).shape, (4,))
        np.testing.assert_equal(
            qa.pow(np.atleast_2d(self.q1), 2).shape, (1, 4))
        np.testing.assert_equal(
            qa.pow(self.q1, [2]).shape, (1, 4))
        np.testing.assert_equal(
            qa.pow(np.atleast_2d(self.q1), [2]).shape, (1, 4))

        np.testing.assert_equal(qa.rotation([0, 0, 1], np.pi).shape, (4,))
        np.testing.assert_equal(
            qa.rotation([[0, 0, 1]], np.pi).shape, (1, 4))
        np.testing.assert_equal(
            qa.rotation([0, 0, 1], [np.pi]).shape, (1, 4))
        np.testing.assert_equal(
            qa.rotation([[0, 0, 1]], [np.pi]).shape, (1, 4))
        ...
        np.testing.assert_equal(
            np.shape(qa.from_angles([0], [0], [0])), (1, 4))
