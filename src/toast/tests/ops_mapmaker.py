...
class MapmakerTest(MPITestCase):
    ...
    #     mapper.apply(data)
    #
    #     print("out amplitudes = ", data["toastmap_solve_amplitudes"][tmpl.name])
    #
    #     # Outputs
    ...
    #             plt.close()
    #
    #             # nt.assert_almost_equal(toast_map[stokes], madam_map[stokes], decimal=6)
    #
    #     del data
    #     return

    def test_compare_madam_prior(self):
        if not ops.Madam.available:
            print("libmadam not available, skipping destriping comparison with prior")
            return

        testdir = os.path.join(self.outdir, "compare_madam_prior")
        if self.comm is None or self.comm.rank == 0:
            os.makedirs(testdir)
