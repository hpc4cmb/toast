...
#pragma GCC diagnostic pop
        ...
        rec->impl = [](function_call &call) -> handle
        {
            ...
            handle result = cast_out::cast(std::move(args_converter).template call<Return, Guard>(cap->f),
                                           policy,
                                           call.parent);
                            ...
                            size_t args)
    {
        ...
        rec->is_constructor = (std::strcmp(rec->name, "__init__") == 0) || (std::strcmp(rec->name, "__setstate__") == 0);
        ...
        for (const auto *pc = text; *pc != '\0'; ++pc)
        {
            ...
            else if (c == '%')
            {
                ...
                else if (rec->is_new_style_constructor && arg_index == 0)
                {
                    ...
                    signature += rec->scope.attr("__module__").cast<std::string>() + "." + rec->scope.attr("__qualname__").cast<std::string>();
                }
                else
                {
        ...
        if (rec->sibling)
        {
            ...
            {
                pybind11_fail("Cannot overload existing non-function object \"" + std::string(rec->name) + "\" with a function of the same name");
            }
        }
        ...
        else
        {
            ...
            if (chain->is_method != rec->is_method)
            {
                pybind11_fail(
                    ...
                    std::string(rec->is_method ? "instance" : "static") + " method " + std::string(pybind11::str(rec->scope.attr("__name__"))) + "." + std::string(rec->name) + signature
#endif
                );
            }
...
public:
    ...
    PYBIND11_NOINLINE void add_object(const char *name, handle obj, bool overwrite = false)
    {
        ...
        {
            pybind11_fail(
                "Error during initialization: multiple incompatible definitions with name \"" + std::string(name) + "\"");
        }
...
protected:
    void initialize(const type_record &rec)
    {
        ...
        {
            pybind11_fail("generic_type: cannot initialize type \"" + std::string(rec.name) + "\": an object with that name is already defined");
        }
                              ...
                              void *get_buffer_data)
    {
        ...
        {
            pybind11_fail("To be able to register buffer protocol support for the type '" + get_fully_qualified_tp_name(tinfo->type) + "' the associated class<>(..) invocation must "
                                  ...
                                  detail::function_record *rec_func)
    {
        ...
        const auto has_doc = (rec_func != nullptr) && (rec_func->doc != nullptr) && pybind11::options::show_user_defined_docstrings();
...
public:
    ...
    class_ &def_readwrite(const char *name, D C::*pm, const Extra &...extra)
    {
        ...
        cpp_function fget([pm](const type &c) -> const D &
                          { return c.*pm; },
                          is_method(*this)),
