...
void init_math_qarray(py::module & m) {
    ...
    m.def(
        "qa_amplitude", [](py::buffer in) {
            pybuffer_check_1D <double> (in);
            py::buffer_info info_in = in.request();
            size_t nquat = (size_t)(info_in.size / 4);
            double * inraw = reinterpret_cast <double *> (info_in.ptr);
            AlignedF64 out(nquat);
            toast::qa_amplitude(nquat, 4, 4, inraw, out.data());
            return py::cast(out);
        }, py::arg(
            "in"), R"(
        Compute the amplitude (norm) of the input quaternions.

        The results are stored in a new output buffer.  To guarantee SIMD
        vectorization, the input array should be aligned
        (i.e. use an AlignedF64).
        ...
        Returns:
            (AlignedF64):  array of amplitudes.
    ...
    m.def(
        "qa_normalize", [](py::buffer in) {
            pybuffer_check_1D <double> (in);
            py::buffer_info info_in = in.request();
            size_t nquat = (size_t)(info_in.size / 4);
            double * inraw = reinterpret_cast <double *> (info_in.ptr);
            AlignedF64 out(nquat * 4);
            toast::qa_normalize(nquat, 4, 4, inraw, out.data());
            return py::cast(out);
        }, py::arg(
            "in"), R"(
        Normalize a quaternion array.

        The results are returned in a new buffer.  To guarantee SIMD
        vectorization, the input array should be aligned
        (i.e. use an AlignedF64).
        ...
        Returns:
            (AlignedF64):  flat-packed normalized output.
    ...
    m.def(
        "qa_rotate", [](py::buffer q_in, py::buffer v_in) {
            pybuffer_check_1D <double> (q_in);
            pybuffer_check_1D <double> (v_in);
            py::buffer_info info_qin = q_in.request();
            py::buffer_info info_vin = v_in.request();
            size_t nquat = (size_t)(info_qin.size / 4);
            size_t nvec = (size_t)(info_vin.size / 3);
            size_t nout = (nquat > nvec) ? nquat : nvec;
            double * qinraw = reinterpret_cast <double *> (info_qin.ptr);
            double * vinraw = reinterpret_cast <double *> (info_vin.ptr);
            AlignedF64 out(nout * 3);
            toast::qa_rotate(nquat, qinraw, nvec, vinraw, out.data());
            return py::cast(out);
        }, py::arg("q_in"), py::arg(
            "v_in"), R"(
        Rotate vectors with quaternions.
        ...
        The results are returned in a new array.  To guarantee SIMD
        ...
        Returns:
            (AlignedF64):  flattened 1D array with the result.
    ...
    m.def(
        "qa_mult", [](py::buffer p_in, py::buffer q_in) {
            pybuffer_check_1D <double> (p_in);
            pybuffer_check_1D <double> (q_in);
            py::buffer_info info_pin = p_in.request();
            py::buffer_info info_qin = q_in.request();
            size_t np = (size_t)(info_pin.size / 4);
            size_t nq = (size_t)(info_qin.size / 4);
            size_t nout = (np > nq) ? np : nq;
            double * pinraw = reinterpret_cast <double *> (info_pin.ptr);
            double * qinraw = reinterpret_cast <double *> (info_qin.ptr);
            AlignedF64 out(nout * 4);
            toast::qa_mult(np, pinraw, nq, qinraw, out.data());
            return py::cast(out);
        }, py::arg("p_in"), py::arg(
            "q_in"), R"(
        Multiply quaternion arrays.
        ...
        Returns:
            (AlignedF64):  The flattened results.
    ...
    m.def(
        "qa_slerp", [](py::buffer time, py::buffer targettime,
            ...
            double * tgtimeraw = reinterpret_cast <double *> (info_tgtime.ptr);
            double * qinraw = reinterpret_cast <double *> (info_qin.ptr);
            AlignedF64 out(ntgtime * 4);
            toast::qa_slerp(ntime, ntgtime, timeraw, tgtimeraw, qinraw,
                            out.data());
            return py::cast(out);
        }, py::arg("time"), py::arg("targettime"), py::arg(
            "q_in"), R"(
        Spherical Linear Interpolation of quaternions.
        ...
        Returns:
            (AlignedF64):  Output interpolated quaternions.
    ...
    m.def(
        "qa_exp", [](py::buffer in) {
            ...
            size_t nin = (size_t)(info_in.size / 4);
            double * inraw = reinterpret_cast <double *> (info_in.ptr);
            AlignedF64 out(nin * 4);
            toast::qa_exp(nin, inraw, out.data());
            return py::cast(out);
        }, py::arg(
            "in"), R"(
        Compute the exponential of a quaternion array.
        ...
        Returns:
            (AlignedF64):  flattened array of output values.
    ...
    m.def(
        "qa_ln", [](py::buffer in) {
            ...
            double * inraw = reinterpret_cast <double *> (info_in.ptr);
            AlignedF64 out(nin * 4);
            toast::qa_ln(nin, inraw, out.data());
            return py::cast(out);
        }, py::arg(
            "in"), R"(
        Compute the natural log of a quaternion array.
    ...
    m.def(
        "qa_pow", [](py::buffer in, py::buffer pw) {
            pybuffer_check_1D <double> (in);
            pybuffer_check_1D <double> (pw);
            py::buffer_info info_in = in.request();
            py::buffer_info info_pw = pw.request();
            ...
            double * inraw = reinterpret_cast <double *> (info_in.ptr);
            AlignedF64 out(nin * 4);
            toast::qa_pow(nin, npw, pwraw, inraw, out.data());
            return py::cast(out);
        }, py::arg("in"), py::arg(
            "pw"), R"(
        Raise a quaternion array to a real power.
    ...
    m.def(
        "qa_from_axisangle", [](py::buffer axis, py::buffer angle) {
            ...
            size_t nang = info_angle.size;
            size_t naxis = (size_t)(info_axis.size / 3);
            size_t nout = (nang > naxis) ? nang : naxis;
            if ((nang != naxis) && (nang != 1) && (naxis != 1)) {
                auto log = toast::Logger::get();
                std::ostringstream o;
                o << "Buffer sizes are not consistent.";
            ...
            double * axisraw = reinterpret_cast <double *> (info_axis.ptr);
            AlignedF64 out(nout * 4);
            toast::qa_from_axisangle(naxis, axisraw, nang, angleraw,
                                     out.data());
            return py::cast(out);
        }, py::arg("axis"), py::arg(
            "angle"), R"(
        ...
        Returns:
            (AlignedF64):  flattened array of output quaternions.
    ...
    m.def(
        "qa_to_axisangle", [](py::buffer q_in) {
            pybuffer_check_1D <double> (q_in);
            py::buffer_info info_qin = q_in.request();
            size_t nq = (size_t)(info_qin.size / 4);
            double * qinraw = reinterpret_cast <double *> (info_qin.ptr);
            AlignedF64 axis(nq * 3);
            AlignedF64 angle(nq);
            toast::qa_to_axisangle(nq, qinraw, axis.data(), angle.data());
            return py::make_tuple(axis, angle);
        }, py::arg(
            "q_in"), R"(
        Convert quaternions into axis / angle representation.

        The results are returned in new output buffers.  To guarantee SIMD
        vectorization, the input array should be aligned
        (i.e. use an AlignedF64).
        ...
        Returns:
            (tuple):  The (axis, angle) flat-packed arrays.
    ...
    m.def(
        "qa_to_rotmat", [](py::buffer q_in) {
            ...
            AlignedF64 mat(nq * 9);
            for (size_t i = 0; i < nq; ++i) {
                toast::qa_to_rotmat(&(qinraw[4 * i]), &(mat.data()[9 * i]));
            }
            return py::cast(mat);
        }, py::arg(
            "q_in"), R"(
        ...
        The results are returned in a new output buffer.  To guarantee SIMD
        vectorization, the input array should be aligned
        (i.e. use an AlignedF64).
        ...
        Returns:
            (AlignedF64):  flattened rotation matrices.
    ...
    m.def(
        "qa_from_rotmat", [](py::buffer mat) {
            pybuffer_check_1D <double> (mat);
            py::buffer_info info_mat = mat.request();
            size_t nmat = (size_t)(info_mat.size / 9);
            double * matraw = reinterpret_cast <double *> (info_mat.ptr);
            AlignedF64 out(nmat * 4);
            for (size_t i = 0; i < nmat; ++i) {
                toast::qa_from_rotmat(&(matraw[9 * i]), &(out.data()[4 * i]));
            }
            return py::cast(out);
        }, py::arg(
            "mat"), R"(
        ...
        Returns:
            (AlignedF64):  flattened output array of quaternions.
    ...
    m.def(
        "qa_from_vectors", [](py::buffer vec1, py::buffer vec2) {
            ...
            double * v1raw = reinterpret_cast <double *> (info_vec1.ptr);
            double * v2raw = reinterpret_cast <double *> (info_vec2.ptr);
            AlignedF64 out(nv1 * 4);
            toast::qa_from_vectors(nv1, v1raw, v2raw, out.data());
            return py::cast(out);
        }, py::arg("vec1"), py::arg(
            "vec2"), R"(
        Create quaternions from two vectors.
        ...
        Returns:
            (AlignedF64):  flattened array of quaternions.
    ...
    m.def(
        "qa_from_angles", [](py::buffer theta, py::buffer phi,
            ...
            double * phiraw = reinterpret_cast <double *> (info_phi.ptr);
            double * paraw = reinterpret_cast <double *> (info_pa.ptr);
            AlignedF64 out(ntheta * 4);
            toast::qa_from_angles(ntheta, thetaraw, phiraw, paraw, out.data(),
                                  IAU);
            return py::cast(out);
        }, py::arg("theta"), py::arg("phi"), py::arg("pa"), py::arg(
            "IAU") = false, R"(
        Create quaternions from spherical coordinates and position angle.
    ...
    m.def(
        "qa_to_angles", [](py::buffer q_in, bool IAU) {
            pybuffer_check_1D <double> (q_in);
            py::buffer_info info_qin = q_in.request();
            size_t nq = (size_t)(info_qin.size / 4);
            double * qinraw = reinterpret_cast <double *> (info_qin.ptr);
            ...
            AlignedF64 phi(nq);
            AlignedF64 pa(nq);
            toast::qa_to_angles(nq, qinraw, theta.data(), phi.data(),
                                pa.data(), IAU);
            return py::make_tuple(theta, phi, pa);
        }, py::arg("q_in"), py::arg(
            "IAU") = false, R"(
        Convert quaternions to spherical coordinates and position angle.
        ...
        The results are returned in new buffers.  To guarantee SIMD
        vectorization, the input array should be aligned
        (i.e. use an AlignedF64).
    ...
    m.def(
        "qa_from_position", [](py::buffer theta, py::buffer phi) {
            ...
            double * phiraw = reinterpret_cast <double *> (info_phi.ptr);
            AlignedF64 out(ntheta * 4);
            toast::qa_from_position(ntheta, thetaraw, phiraw, out.data());
            return py::cast(out);
        }, py::arg("theta"), py::arg(
            "phi"), R"(
        Create quaternions from spherical coordinates.
    ...
    m.def(
        "qa_to_position", [](py::buffer q_in) {
            ...
            AlignedF64 theta(nq);
            AlignedF64 phi(nq);
            toast::qa_to_position(nq, qinraw, theta.data(), phi.data());
