...
pixels_healpix_inner_jax = jax.vmap(
    pixels_healpix_inner_jax, in_axes=[None, 0, None], out_axes=0
...
def pixels_healpix_interval_jax(
    quat_index,
    quats,
    flags,
    flag_mask,
    ...
    print(
        f"DEBUG: jit-compiling 'pixels_healpix_interval_jax' with n_side:{nside} n_det:{quats.shape[0]} n_pix_submap:{n_pix_submap} hit_submaps:{hit_submaps.size} flag_mask:{flag_mask} nest:{nest} intervals_max_length:{intervals_max_length} use_flags:{use_flags}"
    )
...
# jit compiling
pixels_healpix_interval_jax = jax.jit(
    pixels_healpix_interval_jax,
    static_argnames=[
        "flag_mask",
        "n_pix_submap",
        "nside",
...
def pixels_healpix_jax(
    ...
    # runs computation
    new_pixels, new_hit_submaps = pixels_healpix_interval_jax(
        quat_index_input,
        quats_input,
        flags_input,
        flag_mask,
...
def pixels_healpix_numpy(
    ...
    n_samp = pixels.shape[1]
    print(
        f"DEBUG: running 'pixels_healpix_numpy' with n_view:{intervals.size} flag_mask:{flag_mask} n_det:{n_det} n_samp:{n_samp} nest:{nest}"
    )
        ...
        for interval in intervals:
            ...
            for isamp in range(interval_start, interval_end):
                ...
                if use_flags and is_flagged:
                    ...
                    pixel = pixels_healpix_inner_numpy(
