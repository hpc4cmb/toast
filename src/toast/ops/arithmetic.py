    ...
    def _exec(self, data, detectors=None, **kwargs):
        ...
        for ob in data.obs:
            ...
            if self.result == self.first:
                result_units = ob.detdata[self.first].units
                scale_first = 1.0
                scale_second = ob.detdata[self.second].unit_conversion(result_units)
            elif self.result == self.second:
                result_units = ob.detdata[self.second].units
                scale_first = ob.detdata[self.first].unit_conversion(result_units)
                scale_second = 1.0
            else:
