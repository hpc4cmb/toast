...
from ..io.hdf_utils import hdf5_open, hdf5_config
from ..io import save_hdf5_detdata, load_hdf5_detdata
from ..mpi import MPI
from ..observation import default_values as defaults
from ..observation_cache import observation_cache
...
class CacheLoader(object):
    """Loader class to restore signal and flags from an observation cache."""

    def __init__(self, cache_dir, det_data, det_flags):
        self._cache_dir = cache_dir
        self._det_data = det_data
        self._det_flags = det_flags

    @function_timer
    def load(self, obs, **kwargs):
        ob_dir = observation_cache(self._cache_dir, obs, duplicates=None)
        file = os.path.join(ob_dir, f"{self._det_data}.h5")
        if self._det_data in obs.detdata:
            msg = f"Detector data {self._det_data} already exists in {obs.name},"
            msg += " cannot load"
            raise RuntimeError(msg)
        if self._det_flags is not None and self._det_flags in obs.detdata:
            msg = f"Detector flags {self._det_flags} already exists in {obs.name},"
            msg += " cannot load"
            raise RuntimeError(msg)
...
class AccumulateObservation(Operator):
    ...
    If `cache_detdata` is True, then ONLY the signal and flags will be written to disk
    and a new Loader will be added to each observation that will load this in later.
    ...
    def _check_obs_pixel_dist(self, data):
        ...
        if obs_pixel_dist not in data:
            ...
            if hasattr(pix_dist, "nest"):
                data[obs_pixel_dist].nest = pix_dist.nest
            if hasattr(pix_dist, "wcs"):
                data[obs_pixel_dist].wcs = pix_dist.wcs
    ...
    def _check_cache(self, obs):
        if self.cache_dir is None:
            ...
            ob_dir = observation_cache(self.cache_dir, obs, duplicates=None)
            use_cache = True
            if obs.comm.group_rank == 0:
                ...
                if self.overwrite_cache:
                    ...
                    if self.cache_detdata:
                        det_object = os.path.join(
                            ob_dir, f"{self.name}_{self.det_data}.h5"
                        )
                        if os.path.exists(det_object):
                            os.remove(det_object)
    ...
    def _load_obs_products(self, data, ob_dir):
        ...
        if self.cache_detdata:
            ...
            obs_have_obj = True
            if data.comm.group_rank == 0:
                if not os.path.exists(det_object):
                    obs_have_obj = False
            if data.comm.comm_group is not None:
                obs_have_obj = data.comm.comm_group.bcast(obs_have_obj, root=0)
            if not obs_have_obj:
    ...
    def _write_obs_products(self, data, ob_dir, accum_pipe):
        ...
        if self.cache_detdata:
            ...
            if obs.comm.group_size == 1:
                ...
                force_serial = False
            hf = hdf5_open(
                temp_path, "w", comm=data.comm.comm_group, force_serial=force_serial
            )
            fields = [(self.det_data, None)]
            if self.det_flags is not None:
                fields.append((self.det_flags, {"type": "gzip"}))
            save_hdf5_detdata(
                obs,
                hf,
                fields,
                "Cache {obs.name} detdata:",
            )
            if hf is not None:
                hf.close()
            del hf
            ...
            if data.comm.group_rank == 0:
                os.rename(temp_path, det_object)

            # Add a Loader for the future
            obs.loader = CacheLoader(self.cache_dir, self.det_data, None)

            # Store units before deleting
            obs["detector_data_units"] = {
                self.det_data: obs.detdata[self.det_data].units
            }
