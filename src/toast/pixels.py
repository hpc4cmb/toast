...
class PixelData(AcceleratorObject):
    ...
    def write(
        ...
        If no WCS object exists in the PixelDistribution, a Healpix file is written
        ...
        Args:
            ...
            healpix_nest (bool): If True, healpix data is in NESTED ordering, else data
                is in RING ordering.
            hdf5_force_serial (bool): If True, use serial I/O, even if the HDF5
        ...
        if hasattr(dist, "wcs"):
            # We have WCS data.  For both FITS and HDF5 formats, this type of pixel
            ...
            if filename_is_fits(path):
                # When writing to FITS format, we always serialize to the root process.
                ...
                if have_hdf5_parallel() and not hdf5_force_serial:
                    ...
                    if rank == 0:
                        ...
                        with h5py.File(path, "w") as f:
                            ...
                            for rank_send in range(nproc):
                                ...
                                submaps = np.arange(dist.n_submap)[
                                    allowners == rank_send
                                ]
        ...
        if hasattr(dist, "wcs"):
            ...
            if filename_is_fits(path):
                ...
                if rank == 0:
                    ...
                    if nside_map != nside:
                        errors += (
                            f"Wrong NSide: {path} has {nside_map}, expected {nside}\n"
                        )
                ...
                buf = np.zeros(
                    comm_submap * dist.n_pix_submap * self.n_value, dtype=self.dtype
                )
                ...
                while in_off < dist.n_pix:
                    ...
                    if rank == 0:
                        for col in range(self.n_value):
                            ...
                            buf[
                                coloff : coloff
                                + (copyrows * self.n_value) : self.n_value
                            ] = fdata[col][in_off : in_off + copyrows]
                ...
                if rank == 0:
                    ...
                    if nnz < self.n_value:
                        ...
                        raise RuntimeError(msg)
                    if file_nested != healpix_nest:
                        log.warning(
                            f"{path} has ORDERING={header['ORDERING']}, "
                            f"reordering serially upon load."
                        )
                        if file_nested and not healpix_nest:
                            mapdata = hp.reorder(dset[:], n2r=True)
                        elif not file_nested and healpix_nest:
                ...
                buf = np.zeros(
                    comm_submap * self.n_value * dist.n_pix_submap, dtype=self.dtype
                )
                ...
                while submap_offset < n_hit_submaps:
                    ...
                    if rank == 0:
                        for i, submap in enumerate(
                            hit_submaps[submap_offset:submap_last]
                        ):
