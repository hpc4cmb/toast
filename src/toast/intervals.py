...
def build_interval_dtype():
    ...
    offs = [0, dtdbl.alignment, 2 * dtdbl.alignment, 2 * dtdbl.alignment + dtll.alignment]
    return np.dtype(
        {
...
class IntervalList(Sequence):
    ...
    def __invert__(self):
        ...
        return IntervalList(
            self.timestamps, intervals=np.array(neg, dtype=interval_dtype).view(np.recarray)
        )
    ...
    def __and__(self, other):
        ...
        return IntervalList(
            self.timestamps, intervals=np.array(result, dtype=interval_dtype).view(np.recarray)
        )
