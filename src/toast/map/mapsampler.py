...
#
# from ..ctoast import map_tools_fast_scanning32 as fast_scanning32
# import warnings
#
...
#     """
#
#     def __init__(self, map_path, pol=False, pol_fwhm=None,
#                  no_temperature=False, dtype=None,
#                  verbose=False, nside=None, comm=None, cache=None,
#                  preloaded_map=None, buflen=1000000, nest=False):
#         """
...
#             raise RuntimeError('You cannot have pol=False, '
#                                'no_temperature=True')
#
#         self.path = map_path
#         self.pol = pol
...
#             warnings.warn('MapSampler no longer supports dtype',
#                           DeprecationWarning)
#
...
#                     if no_temperature:
#                         (self._map_Q,
#                          self._map_U) = np.array(preloaded_map, dtype=DTYPE)
#                     else:
#                         (self._map, self._map_Q,
#                          self._map_U) = np.array(preloaded_map, dtype=DTYPE)
#                 else:
#                     if no_temperature:
#                         self._map_Q, self._map_U = hp.read_map(
#                             self.path, field=[1, 2], dtype=DTYPE,
#                             verbose=verbose, memmmap=True, nest=self.nest)
#                     else:
#                         try:
#                             self._map, self._map_Q, self._map_U = hp.read_map(
#                                 self.path, field=[0, 1, 2], dtype=DTYPE,
#                                 verbose=verbose, memmap=True, nest=self.nest)
#                         except IndexError:
#                             print('WARNING: {} is not polarized'.format(
#                                 self.path), flush=True)
#                             self.pol = False
#                             self._map = hp.read_map(
#                                 self.path, dtype=DTYPE,
#                                 verbose=verbose, memmap=True, nest=self.nest)
#
#                 if nside is not None:
#                     if not no_temperature:
#                         self._map = hp.ud_grade(
#                             self._map, nside, dtype=DTYPE,
#                             order_in=self.order, order_out=self.order)
#                     if self.pol:
#                         self._map_Q = hp.ud_grade(
#                             self._map_Q, nside, dtype=DTYPE,
#                             order_in=self.order, order_out=self.order)
#                         self._map_U = hp.ud_grade(
#                             self._map_U, nside, dtype=DTYPE, order_in=self.order,
...
#                 else:
#                     self._map = hp.read_map(map_path, field=[0],
#                                            dtype=DTYPE, verbose=verbose,
#                                            memmap=True, nest=self.nest)
#                 if nside is not None:
#                     self._map = hp.ud_grade(
#                         self._map, nside, dtype=DTYPE, order_in=self.order,
#                         order_out=self.order)
#                 plug_holes(self._map, verbose=verbose, nest=self.nest)
#
#         if self.ntask > 1:
