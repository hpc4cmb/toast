...
def distribute_discrete(sizes, groups, pow=1.0, breaks=None):
    ...
    max_per_proc = float(distribute_partition(weights.astype(np.int64),
                                              groups))
    ...
    if len(dist) != groups:
        raise RuntimeError("Number of distributed groups different than "
                           "number requested")
    return dist
...
def distribute_samples(mpicomm, detectors, samples, detranks=1,
        ...
        for ob in self.obs:
            ...
            if tod is not None:
                ...
                procstr = "{}    sample range {} --> {} in {} chunks:\n"\
                    .format(procstr, offset, (offset + nsamp - 1), my_chunks)
                ...
                if nsamp > 0:
                    ...
                    for dt in dets:
                        ...
                        procstr = \
                            "{}      pntg [{:.3e} {:.3e} {:.3e} {:.3e}] "\
                            "--> [{:.3e} {:.3e} {:.3e} {:.3e}]\n"\
                            .format(procstr, pdata[0, 0], pdata[0, 1],
                                    pdata[0, 2], pdata[0, 3], pdata[-1, 0],
                                    pdata[-1, 1], pdata[-1, 2], pdata[-1, 3])

                        data = tod.local_signal(dt)
                        flags = tod.local_flags(dt)
                        procstr = \
                            "{}      {:.3e} ({}) --> {:.3e} ({})\n"\
                            .format(procstr, data[0], flags[0], data[-1],
                                    flags[-1])
