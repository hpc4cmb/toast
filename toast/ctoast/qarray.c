...
//
// Here's how to allocate an array of quaternions: (row-major)
// $~> double* q = malloc( n * 4 * sizeof(double) );
//
// And of course, free it (bad memory leaks!):
// $~> free(q);
...
void qmult(const int n, const double* p, const double* q, double* r) {
  for (i = 0; i < n; ++i)
  {
    r[4*i + 0] =  p[4*i + 0] * q[4*i + 3] + p[4*i + 1] * q[4*i + 2] - p[4*i + 2] * q[4*i + 1] + p[4*i + 3] * q[4*i + 0];
    r[4*i + 1] = -p[4*i + 0] * q[4*i + 2] + p[4*i + 1] * q[4*i + 3] + p[4*i + 2] * q[4*i + 0] + p[4*i + 3] * q[4*i + 1];
    r[4*i + 2] =  p[4*i + 0] * q[4*i + 1] - p[4*i + 1] * q[4*i + 0] + p[4*i + 2] * q[4*i + 3] + p[4*i + 3] * q[4*i + 2];
    r[4*i + 3] = -p[4*i + 0] * q[4*i + 0] - p[4*i + 1] * q[4*i + 1] - p[4*i + 2] * q[4*i + 2] + p[4*i + 3] * q[4*i + 3];
  }
}
...
void nlerp(const int n_time, const double* targettime, const double* time, const double* q_in, double* q_interp) {
  ...
  qnorm_inplace(n_time, 4, q_interp);
...
void slerp(const int n_time, const double* targettime, const double* time, const double* q_in, double* q_interp) {
  ...
  // Think of removing first and last row from all the arrays (since not interpolated)
  for (i = 0; i < n_time; ++i)
  {
    for (j = 0; j < 4; ++j)
    {
  ...
  qinv(n_time, q_tmp2);
  qmult(n_time, q_tmp1, q_tmp2, q_interp);
  qpow(n_time, t_matrix, q_interp, q_tmp1);
  qinv(n_time, q_tmp2); // q_tmp2 back to where it was at first
  qmult(n_time, q_tmp1, q_tmp2, q_interp);
...
void qexp(const int n, const double* q_in, double* q_out) {
  ...
  qamplitude(n, 3, q_in, normv);
  // Think of taking the exp() out of the loop and do it only once
  double exp_q_w;
  for (i = 0; i < n; ++i)
  {
...
void qln(const int n, const double* q_in, double* q_out) {
  ...
  qamplitude(n, 4, q_in, normq);
  for (i = 0; i < n; ++i)
  {
    q_out[4*i + 3] = log(normq[i]);
  }
  qnorm(n, 3, q_in, q_out);
  double tmp;
  for (i = 0; i < n; ++i)
  {
