...
PREFIX="${HOME}/software/toastdeps"
VERSION=$(date "+%Y%m%d")

PREFIX_AUX="${PREFIX}/${VERSION}/aux"
PREFIX_CONDA="${PREFIX}/${VERSION}/conda"
...
# Install SPT3G.
 ...
 git clone https://github.com/CMB-S4/spt3g_software.git --branch master --single-branch --depth 1 \
    ...
    && cd .. \
    && cp -a spt3g_software "${PREFIX_AUX}/spt3g" \
    && cd "${PREFIX_AUX}/spt3g" \
    && mkdir build \
    && cd build \
